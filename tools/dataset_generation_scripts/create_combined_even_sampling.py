...
def evenly_spread_sample(train_seq, n_datapoints, clip_idx=None):
    # Determine the number of sequences and the spread interval
    num_sequences = len(real_train_seqs)
    num_samples = max(1, n_datapoints // num_sequences)
    
    # If all data is needed, return all data insted of trying to sample it
    if np.ceil(n_datapoints / num_sequences) >= len(train_seq[0]):
        print("WARNING: Returning all data of this sequence!") # Warning to make user aware, as it might not be the desired outcome
        return train_seq[0], train_seq[1]

    # Select evenly spaced indices
    indices = np.linspace(0, len(train_seq[0])-1, num_samples).round().astype(np.int16)
    indices = np.unique(indices)
    
    # If the clip_idx var is set, clip the amount of indecies used
    if clip_idx is not None:
        indices = indices[:clip_idx]
    
    # Sample data and labels using the computed indices
    cropped_train_seq_points = np.array(train_seq[0])[indices]
    cropped_train_seq_labels = np.array(train_seq[1])[indices]

    return cropped_train_seq_points.tolist(), cropped_train_seq_labels.tolist()
...
for n_datapoints in n_datapoints_list:
    ...
    save_root = save_dir / "tractors_and_combines_combined_even_sampling" / f"{n_datapoints}" / "dataset" / "sets"
    ...
    for train_seq in zip(real_train_points, real_train_labels):
        ...
        cropped_train_seq_points, cropped_train_seq_labels = evenly_spread_sample(train_seq, n_datapoints)
        ...
        if round(upsample_ratio % 1, 2) >= 0.01:
            ...
            # Envenly sample the missing points
            missing_train_seq = evenly_spread_sample(train_seq, n_points_missing, clip_idx=n_points_missing)
            
            # Symlink missing amount
            symlinker(path_list=missing_train_seq[0], new_seq_path=save_root, prefix=prefix + 1)
            symlinker(path_list=missing_train_seq[1], new_seq_path=save_root, prefix=prefix + 1)
